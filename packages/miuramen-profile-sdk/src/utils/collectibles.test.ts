...
const MIUX_NFT_ADDRESS = "0xDf7952B35f24aCF7fC0487D01c8d5690a60DBa07";
...
describe("collectibles", () => {
  it("getIdentifierKeyFromAddress returns proper identifier key", () => {
    const miuramenIdentifierKey = getIdentifierKeyFromAddress(MIUX_NFT_ADDRESS, MAINNET_CHAIN_ID);
    const mixieIdentifierKey = getIdentifierKeyFromAddress(MIXIE_NFT_ADDRESS, MAINNET_CHAIN_ID);
    expect(miuramenIdentifierKey).toBe("image");
  ...
  it("getTokenUriData returns proper response", async () => {
    const uriData = await getTokenUriData(MIUX_NFT_ADDRESS, MOCK_TOKEN_ID, simpleRpcProvider);
    expect(uriData).toEqual({
      name: "Sleepy",
      description: "Aww, looks like eating miuramens all day is tough work. Sweet dreams!",
  ...
  it("getNftByTokenId returns proper nft", async () => {
    ...
    const nft = await getNftByTokenId(MIUX_NFT_ADDRESS, MOCK_TOKEN_ID, simpleRpcProvider, MAINNET_CHAIN_ID);
