# MiuRamenswap Profile SDK

This package provides some handy functions to retrieve data for MiuRamenswap Profile system.
...
Install `@miuramenswap/profile-sdk` into your project with npm:

```bash
npm install @miuramenswap/profile-sdk --save
```

or yarn:

```bash
yarn add @miuramenswap/profile-sdk
```
...
```js
import MiuRamenProfileSdk from "@miuramenswap/profile-sdk";

const miuramenSdk = new MiuRamenProfileSdk();
```
...
const miuramenSdk = new MiuRamenProfileSdk(customProvider, 97);
```
...
const miuramenSdk = new MiuRamenProfileSdk();
const username = miuramenSdk.getUsername("0x123456789");
...
const miuramenSdk = new MiuRamenProfileSdk();
const team = miuramenSdk.getTeam(1);
console.log(team);
// {
//   id: 1,
//   name: "Syrup Storm",
...
It also sets `profile_${address}` cookie containing username and avatar (now only for miuramenswap.finance domain, maybe configurable in future versions)

```js
import MiuRamenProfileSdk from "@miuramenswap/profile-sdk";

const miuramenSdk = new MiuRamenProfileSdk();
const profile = miuramenSdk.getProfile("0x123456789");
console.log(profile);
// {
//   hasRegistered: true
//   profile: {
...
const achievements = miuramenSdk.getAchievements("0x123456789");
...
```JSX
import { Team, teamImages } from "@miuramenswap/profile-sdk";
...
Current version of this SDK is 90% copy of existing from [miuramen-frontend](https://github.com/miuramenswap/miuramen-frontend) repo. There are several improvements to be made in the future versions of this SDK:
