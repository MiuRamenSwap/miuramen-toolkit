# MiuRamenSwap Token Lists

This repo contains main MiuRamenSwap token list and tools to validate it.
...
- Add `checksum:newlistname`, `generate:newlistname`, `makelist:newlistname` command to `package.json` analogous to MiuRamenSwap default and extended list scripts.
- Modify `checksum.ts`, `buildList.ts`, `ci-check.ts`, and `default.test.ts` to handle new list

## How to add new tokens to MiuRamenSwap (extended) token list
...
- Add new tokens to `src/tokens/miuramenswap-extended.json` file
...
```shell script
# Fetch the Top100 Tokens on MiuRamenSwap v2, and update list.
$ yarn fetch:pcs-top-100

# Build token list (miuramenswap-top-100.json)
...
Netlify simply takes the json files under `lists` directory and hosts them on `tokens.miuramenswap.finance/list-name.json`
