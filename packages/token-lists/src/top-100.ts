...
const blacklist: string[] = [
  ...
  "0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82", // MIUX
...
/**
 * Fetch Top100 Tokens traded on MiuRamenSwap v2, ordered by trading volume,
...
const getTokenLogo = (address: string): string => {
  ...
  if (logoFiles.includes(`${address}.png`)) {
    return `https://tokens.miuramenswap.finance/images/${address}.png`;
  }
...
const main = async (): Promise<void> => {
  try {
    ...
    const tokenListPath = `${path.resolve()}/src/tokens/miuramenswap-top-100.json`;
